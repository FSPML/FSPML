...
//			point.setBaseVerValue(point.getVerValue());
//		}else if(iteNum == 3){
//			context.setVertexAgg(Math.log(curErrSum));
//		}else if(iteNum == startRepeatIte+2){
//			lastErrVals = point.getBaseVerValue().getErrors();
//			for(int j=0; j<k; j++) {
//				lastErrSum += lastErrVals[j];
//			}
...
//		if(!point.isFirstCompute()) { //非第一遍处理数据
//			if (context.getSuperstepCounter() == 3 || lastErrSum==0.0) {
//				context.setVertexAgg(Math.log(curErrSum));
//			}else if(context.getSuperstepCounter() != 3){
//				context.setVertexAgg((Math.log(curErrSum)-Math.log(lastErrSum)));
//			}
//		}else {
//			point.setFirstCompute(false);
