...
public class FCMBSP extends BSP<FCMValue, Integer, Integer, Integer> {
	...
	public void update(
		...
		double[] last_val_miu = (point.isFirstCompute()? 
		...
		if(iteNum <= startRepeatIte+1) {
			FCMValue baseValue = new FCMValue(numOfCentroids);
			baseValue.set(last_fuzzy_error, last_val_miu);
			point.setBaseVerValue(baseValue);
			/** update errors */
			...
			context.setVertexAgg(cur_fuzzy_error - point.getBaseVerValue().getError());
		}else {
			/** update errors */
			context.setVertexAgg(cur_fuzzy_error-last_fuzzy_error);
