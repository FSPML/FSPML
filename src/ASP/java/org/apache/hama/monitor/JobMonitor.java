...
public class JobMonitor {
	...
	public synchronized void updateMonitor(int curIteNum, int taskId, double aggTask, Counters counters, int startRepeatIte) {
		if (this.agg_list.size() < curIteNum) { // first task
			...
			/*
			 * Counters acc_counters = this.counters_list.get(idx); for (Enum<?> name:
			 * COUNTER.values()) { acc_counters.addCounter(name, counters.getCounter(name));
			 * }
			 */
		}
	...
	public void computeAgg(int curIteNum) {
		if (curIteNum == 3) {
			int idx = curIteNum - 1;
			this.agg_list.set(idx, this.agg_list.get(idx)); 
		}else if (curIteNum == this.startRepeatIte+2) {
			int idx = curIteNum - 1;
			this.agg_list.set(idx, this.agg_list.get(this.startRepeatIte-2) + this.agg_list.get(idx));
		} else if (curIteNum == this.startRepeatIte+3) {
			int idx = curIteNum - 1;
			this.agg_list.set(idx, this.agg_list.get(this.startRepeatIte) + this.agg_list.get(idx));
		} else if (curIteNum > 1) {
			int idx = curIteNum - 1;
			this.agg_list.set(idx, this.agg_list.get(idx - 1) + this.agg_list.get(idx));
		}
	}
	...
	public ArrayList<Double> getAggList(){
		return this.agg_list;
	}
