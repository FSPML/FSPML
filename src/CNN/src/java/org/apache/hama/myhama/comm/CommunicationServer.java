...
public class CommunicationServer<V, W, M, I> 
	...
	private volatile Integer semiMutex = 0;
	private boolean semiHasNotify = false;
	...
	public void barrierOfSemiAsyn() throws Exception {
		synchronized(semiMutex) {
			if (!this.semiHasNotify) {
				semiMutex.wait();
			}
			
			this.semiHasNotify = false;
		}
	}
	
	@Override
	public void setCentroids(CenterSetOfKmeans _csk, int blockNum) {
		this.ssc.setGlobalParameters(_csk);
		this.ssc.setBlockNum(blockNum);
		
		synchronized(semiAsynBarrier) {
			this.semiAsynBarrier = false;
		}
		
		synchronized(this.semiMutex) {
			this.semiHasNotify = true;
			this.semiMutex.notify();
		}
	}
