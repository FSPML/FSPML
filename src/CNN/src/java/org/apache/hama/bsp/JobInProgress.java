...
class JobInProgress {
	...
	private DeltaParaOfCNN[] deltaParaOfTasks;
	...
	public JobInProgress(BSPJobID _jobId, Path _jobFile, BSPMaster _master,
		...
		this.deltaParaOfTasks = new DeltaParaOfCNN[taskNum];
	}
	...
	public void registerTask(TaskInformation tif) {
		...
		if (finished == this.taskNum) {
			...
			ssc.setGlobalCNNParameters(new ParameterTransfer());
	...
	private SuperStepCommand updatePara() {
		SuperStepCommand ssc = new SuperStepCommand();
		ParameterTransfer parameterTransfer = new ParameterTransfer();
//		this.deltaParaOfTasks;
		parameterTransfer.setAllHasPro(this.haspro);
		if(this.deltaParaOfTasks[0].getDeltaKernels() != null) {
			int mpaNum = this.deltaParaOfTasks[0].getDeltaKernels()[0].length;
			int lastMapNum = this.deltaParaOfTasks[0].getDeltaKernels().length;
			// 初始化卷积核，共有frontMapNum*outMapNum个卷积核
			double[][][][] deltaKernels = new double[lastMapNum][mpaNum][][];
						LOG.info("mpaNum = " + mpaNum + " lastMapNum= " + lastMapNum);

			for (int j = 0; j < mpaNum; j++) {
				for (int i = 0; i < lastMapNum; i++) {
					// 对batch的每个记录delta求和
					double[][] deltaKernel = this.deltaParaOfTasks[0].getDeltaKernels()[i][j];
					for (int r = 1; r < deltaParaOfTasks.length; r++) {
						// 累积求和
						deltaKernel = Util.matrixOp(this.deltaParaOfTasks[r].getDeltaKernels()[i][j],
									deltaKernel, null, null, Util.plus);
						
					}
					deltaKernels[i][j] =deltaKernel;
				}
			}
			parameterTransfer.setKernel(deltaKernels);
		}
		
		
		if(this.deltaParaOfTasks[0].getDeltaBias() != null){
			int len = this.deltaParaOfTasks[0].getDeltaBias().length;
			double[] deltaBias = new double[len];
			for (int i = 0; i < len; i++) {
				double deltaBia = this.deltaParaOfTasks[0].getDeltaBias()[i];
				for (int r = 1; r < deltaParaOfTasks.length; r++) {
					// 累积求和
					deltaBia += this.deltaParaOfTasks[r].getDeltaBias()[i];
				}
				deltaBias[i] = deltaBia / len;
			}
			parameterTransfer.setBias(deltaBias);
		}
		
		ssc.setGlobalCNNParameters(parameterTransfer);
		ssc.setCommandType(CommandType.START);
	 ...
	 * @param x 
	 */
	public void initConvPara(int parId, int frontMapNum, int outMapNum, int x) {
		// TODO Auto-generated method stub
		int finished = this.reportCounter.incrementAndGet();
		...
		if (finished == this.taskNum) {
			...
			SuperStepCommand ssc = getConvPara(frontMapNum, outMapNum, x);
			for (Entry<Integer, CommunicationServerProtocol> entry: this.comms.entrySet()) {
				try {
					entry.getValue().setNextSuperStepCommand(ssc);
				} catch (Exception e) {
	...
	private SuperStepCommand getConvPara(int frontMapNum, int outMapNum, int x) {
        SuperStepCommand ssc = new SuperStepCommand();
        ParameterTransfer parameterTransfer = new ParameterTransfer();
        initRandomConvPara(parameterTransfer, frontMapNum, outMapNum, x);
        ssc.setGlobalCNNParameters(parameterTransfer);
		ssc.setCommandType(CommandType.START);
		return ssc;
	}

	private void initRandomConvPara(ParameterTransfer parameterTransfer, int frontMapNum, int outMapNum, int x) {
		// 初始化卷积核，共有frontMapNum*outMapNum个卷积核
		double[][][][] kernel = new double[frontMapNum][outMapNum][x][x];
		for (int i = 0; i < frontMapNum; i++)
			for (int j = 0; j < outMapNum; j++)
				kernel[i][j] = Util.randomMatrix(x, x,true);
		// 初始化偏置，共有frontMapNum*outMapNum个偏置
		double[] bias = Util.randomArray(outMapNum);
		
		parameterTransfer.setPara(kernel, bias);
	}

	public void initOutputPara(int parId, int frontMapNum, int outMapNum, int x) {
		// TODO Auto-generated method stub
		// TODO Auto-generated method stub
		int finished = this.reportCounter.incrementAndGet();
		//LOG.info("[SUPERSTEP] taskID=" + parId + ", finished=" + finished);
		if (finished == this.taskNum) {
			...
			SuperStepCommand ssc = getOutputPara(frontMapNum, outMapNum, x);
			for (Entry<Integer, CommunicationServerProtocol> entry: this.comms.entrySet()) {
				try {
					entry.getValue().setNextSuperStepCommand(ssc);
				} catch (Exception e) {
	...
	private SuperStepCommand getOutputPara(int frontMapNum, int outMapNum, int x) {
        SuperStepCommand ssc = new SuperStepCommand();
        ParameterTransfer parameterTransfer = new ParameterTransfer();
        initRandomOutputPara(parameterTransfer, frontMapNum, outMapNum, x);
        ssc.setGlobalCNNParameters(parameterTransfer);
		ssc.setCommandType(CommandType.START);
		return ssc;
	}
	private void initRandomOutputPara(ParameterTransfer parameterTransfer, int frontMapNum, int outMapNum, int x) {
		// 初始化卷积核，共有frontMapNum*outMapNum个卷积核
		double[][][][] kernel = new double[frontMapNum][outMapNum][x][x];
		for (int i = 0; i < frontMapNum; i++)
			for (int j = 0; j < outMapNum; j++)
				kernel[i][j] = Util.randomMatrix(x, x,false);
		// 初始化偏置，共有frontMapNum*outMapNum个偏置
		double[] bias = Util.randomArray(outMapNum);
	...
	public void updateBiasANDKernels(int parId, SuperStepReport ssr) {

		//此处获得的是用于更新的值，如梯度
		this.deltaParaOfTasks[parId] = ssr.getDeltaPara();
		this.haspro += ssr.getHaspro();
		
		int finished = this.reportCounter.incrementAndGet();
		if (finished == this.taskNum) {
			this.reportCounter.set(0);
			SuperStepCommand ssc = updatePara();
