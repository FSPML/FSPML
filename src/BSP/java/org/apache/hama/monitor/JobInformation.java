...
public class JobInformation implements Writable {
	...
	private int[] verMinIds, verMaxIds, verNums;
	...
	private void initVerIdAndNums(int _verNum) {
		...
		for (int i = 0; i < taskNum; i++) {
			this.verNums[i] = this.verMaxIds[i] - this.verMinIds[i];
		}
	...
	/**
	 * Ensure that every worker has the same #blocks, otherwise, 
	 * some workers will initiate one more barrier to update centers in BLOCK.
	 * @return
	 */
	public int refineBlockSize(int blockSize) {
		double[] numOfBlocks = new double[this.taskNum];
		double min = Double.MAX_VALUE, max = 0.0;
		int maxIdx = -1;
		for (int i = 0; i < this.taskNum; i++) {
			numOfBlocks[i] = this.verNums[i] / (double)blockSize;
			if (min > numOfBlocks[i]) {
				min = numOfBlocks[i];
			}
			
			if (max < numOfBlocks[i]) {
				max = numOfBlocks[i];
				maxIdx = i;
			}
		}
		...
		if (Math.floor(min) == Math.floor(max)) {
			LOG.info("original blockSize is OK");
			return blockSize;
		}
		
		LOG.info("original blockSize=" + blockSize + ", #blocks=\n" 
				+ Arrays.toString(numOfBlocks) 
				+ "\nmin=" + min + ", max=" + max);
		int refined = (int)Math.ceil(this.verNums[maxIdx]/(double)Math.floor(max));
		
		min = Double.MAX_VALUE;
		max = 0.0;
		for (int i = 0; i < this.taskNum; i++) {
			numOfBlocks[i] = this.verNums[i] / (double)refined;
			min = min<numOfBlocks[i]? min:numOfBlocks[i];
			max = max>numOfBlocks[i]? max:numOfBlocks[i];
		}
		LOG.info("refined blockSize=" + refined + ", #blocks=\n" 
				+ Arrays.toString(numOfBlocks) 
				+ "\nmin=" + min + ", max=" + max);
		return refined;
	}
	...
	public void recordIterationInformation(String command) {
