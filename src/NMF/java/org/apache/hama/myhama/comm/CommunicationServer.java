...
public class CommunicationServer<V, W, M, I> 
	...
	public void sendPoints(int totalOfSend) {
		...
		InetSocketAddress[] toAddresses = new InetSocketAddress[3];
		CommunicationServerProtocol[] comms = new CommunicationServerProtocol[3];
		...
		while (totalOfSend > 0) {
			...
			for (int i = 0; i < 3; i++) {
				LOG.info("this.parId = " + this.parId + "===============================");
				GraphRecord<V, W, M, I>[] points = this.graphDataServer.selectPointsTosend(unitOfOneSend / 3);
