...
class JobInProgress {
	...
	private MatrixOfNMF matrixs;
	private MatrixOfNMF[] matrixOfNMFOfTask;
	private double[][] sumOfUpdateH;
	...
	public JobInProgress(BSPJobID _jobId, Path _jobFile, BSPMaster _master,
		...
		} else if (this.job.isNMF()) {
			this.matrixOfNMFOfTask = new MatrixOfNMF[taskNum];
			this.matrixs = new MatrixOfNMF(this.numOfDimensions);
			this.sumOfUpdateH = new double[6][this.numOfDimensions];
		}else {
	...
	public void registerTask(TaskInformation tif) {
		...
		if (tif.getTaskId() == 0) {
			if (this.job.isFCM()) {
				...
				double[][] initH = new double[6][this.numOfDimensions];
				Random random = new Random();
				// 随机初始化矩阵 H
				for (int i = 0; i < 6; i++) {
					for (int j = 0; j < this.numOfDimensions; j++) {
						initH[i][j] = random.nextDouble() * 10.0 + random.nextDouble();
					}
				}
				this.csk.setMatrixH(initH);
			} else {
...
//			}
	...
	public void finishSuperStep(int parId, SuperStepReport ssr, String host) {
		...
		if (this.job.isLOGR()) {//此处获得的是用于更新的值，如梯度
			...
			this.matrixOfNMFOfTask[parId] = ssr.getMatSet();
		} else {
		...
		if(finished == 1 || finished == 2 || finished == 3 || finished == 4){
			LOG.info("had report task id is :" + parId);
			LOG.info("first ite time:" + (System.currentTimeMillis() - this.startTimeIte));
		...
		if (finished == this.taskNum) {
			...
			this.jobInfo.recordIterationRuntime((System.currentTimeMillis() - this.startTimeIte) / 1000.0); 
			this.startTimeIte = System.currentTimeMillis();
			...
			if (this.syncTime > 0) {
				this.interAdj.updateSyncTime(System.currentTimeMillis() - finishTime);
			}
	...
	private void updateCenters() {
		if (this.job.isLOGR()) {
			...
			int hr = this.matrixs.getMatrixUpdateH().length;
			int hc = this.matrixs.getMatrixUpdateH()[0].length;
			
			for (int i = 0; i < hr; i++) {
				for (int j = 0; j < hc; j++) {
					this.sumOfUpdateH[i][j] = 0.0;
					for (int k = 0; k < taskNum; k++) {
						this.sumOfUpdateH[i][j] += this.matrixOfNMFOfTask[k].getMatrixUpdateH()[i][j];
					}
				}
			}
			
			this.csk.updateMatrixH(this.sumOfUpdateH);
			this.matrixs.reset();
		} else {
